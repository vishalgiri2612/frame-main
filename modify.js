...
const p = path.resolve('c:/Users/visha/OneDrive/Desktop/frame/frame-main/components/sections/FeaturedFrames.jsx');
let content = fs.readFileSync(p, 'utf8');

content = content.replace(`import { useState } from 'react';`, `import { useState, useEffect } from 'react';`);

const compStr = `const ProductCardItem = ({ product, index, toggleWishlist, isInWishlist, addToCart }) => {
...
content = content.replace('export default function FeaturedFrames', compStr);

const mapRegex = /\{filteredProducts\.map\(\(\s*product,\s*i\s*\)\s*=>\s*\([\s\S]*?\)\s*\)\}/;
const mapReplacement = \`{filteredProducts.map((product, i) => (
            <ProductCardItem
