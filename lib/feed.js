...
export function processProduct(p) {
  ...
  const searchStr = ` ${p.name} ${p.description || ''} ${p.category} `.toUpperCase();
  let gender = 'UNISEX'; // Default
  
  const isMen = /\b(MEN|MALE|GENTS|MAN)\b/.test(searchStr);
  const isWomen = /\b(WOMEN|FEMALE|LADIES|WOMAN)\b/.test(searchStr);
  const isKids = /\b(KIDS|CHILD|BOY|GIRL|JUNIOR)\b/.test(searchStr);
  const isUnisex = /\bUNISEX\b/.test(searchStr);
  ...
  return {
    ...
    brandSlug,
    categorySlug,
    gender, // New field for filtering
...
export function getCategories(productsArray = []) {
  ...
  for (const product of productsArray) {
    if (product.gender) gendersFound.add(product.gender);
  ...
  // Add gender-based categories first
  ['MEN', 'WOMEN', 'KIDS'].forEach(g => {
    if (gendersFound.has(g) || g === 'MEN' || g === 'WOMEN') { // Always show MEN/WOMEN
      result.push({
        slug: g.toLowerCase(),
        name: g,
        count: productsArray.filter(p => {
          if (g === 'MEN') return p.gender === 'MEN' || p.gender === 'UNISEX' || p.gender === 'KIDS';
          if (g === 'WOMEN') return p.gender === 'WOMEN' || p.gender === 'UNISEX' || p.gender === 'KIDS';
          if (g === 'KIDS') return p.gender === 'KIDS';
          return false;
        }).length
      });
    }
  ...
  const otherCats = Array.from(categories.values())
    .filter(c => !['MEN', 'WOMEN', 'KIDS'].includes(c.name))
