...
export default function ShopMain({ 
  ...
  const [sortBy, setSort] = useState(initialSort);
  ...
  }, [initialCategory, initialBrand, initialSort]);
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === 'price_asc') {
      ...
      list.sort((a, b) => {
        // Prioritize manually flagged new arrivals
        if (a.newArrival && !b.newArrival) return -1;
        if (!a.newArrival && b.newArrival) return 1;
        
        const dateA = a.createdAt ? new Date(a.createdAt) : new Date(parseInt((a._id || a.id).toString().substring(0, 8), 16) * 1000);
        const dateB = b.createdAt ? new Date(b.createdAt) : new Date(parseInt((b._id || b.id).toString().substring(0, 8), 16) * 1000);
        return dateB - dateA;
      });
    }
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 sm:gap-8">
            ...
            >
              ...
              <span className="italic text-gold" style={{ fontFamily: 'var(--font-cormorant)' }}>
                {sortBy === 'newest' ? 'ARRIVALS.' : 'MAISONS.'}
              </span>
            </motion.h1>
